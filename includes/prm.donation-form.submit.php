...
function prm_donation_form_validate($post) {
	...
	if (!empty($error)) {
		error_log(sprintf('PRM: Invalid form submission: %s', print_r($error, TRUE)));
		return $error;
	}
}
...
function prm_donation_form_submit_save($values) {
	...
	if ($insert) {
		...
		error_log(sprintf('PRM: Error while saving subscription data: %s', $wpdb->last_error));
	}
}
...
function prm_donation_form_submit_payment_pagseguro($id, $values) {
	...
	if (isset($pre_approval['body']) && $pre_approval_obj = new SimpleXMLElement($pre_approval['body'])) {
		...
		error_log(sprintf("PRM: PagSeguro request failed. Request url: %s \n Request arguments: %s \n Response body: %s", $pagseguro_pre_url, print_r($args, TRUE), print_r($pre_approval['body'], TRUE)));
	}
}
